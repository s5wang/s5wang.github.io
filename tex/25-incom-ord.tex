...
One immediate consequence of this, as we shall comment at the end, is that the existence of any non-classical model of $\mathrm{IZF} + V \neq L$ will imply $\mathrm{IZF} \nvdash \Ord \subseteq L$, and will resolve the long-standing open problem mentioned above.
...
Let $\mathrm{IZF}$ denote the intuitionistic theory with the same axioms as $\mathrm{ZF}$ except the axiom of set induction is substituted for foundation. We shall conclude by showing that \autoref{thm:main-equiv} implies finding any non-classical model of $\mathrm{IZF} + V \neq L$ would immediate disprove $\mathrm{IZF} \vdash \Ord \subseteq L$. More precisely,

\begin{corollary}
  Let $\varphi$ be any formula such that $\mathrm{ZF} \vdash \neg \varphi$, if $\mathrm{IZF} + V \neq L + \varphi$ is consistent, then $\mathrm{IZF} \nvdash \Ord \subseteq L$.
\end{corollary}

\begin{proof}
  We assume that $\mathrm{IZF} \vdash \Ord \subseteq L$. Let $T$ denote the theory $\mathrm{IZF} + V \neq L + \varphi$. For an arbitrary formula $\psi\mleft(x_1, \ldots, x_n\mright)$ with all free variables listed, we can consider the set
  ...
  \end{align*}
  In other words, $\neg \forall x_1, \ldots, x_n \left(\psi\mleft(x_1, \ldots, x_n\mright) \lor \neg \psi\mleft(x_1, \ldots, x_n\mright)\right)$ implies $\alpha_\psi \perp 2$ and thus, by \autoref{thm:main-equiv}, $\mathrm{IZF} + \neg \forall x_1, \ldots, x_n \left(\psi\mleft(x_1, \ldots, x_n\mright) \lor \neg \psi\mleft(x_1, \ldots, x_n\mright)\right) \vdash V = L$, i.e.\
  \[T \vdash \neg \neg \forall x_1, \ldots, x_n \left(\psi\mleft(x_1, \ldots, x_n\mright) \lor \neg \psi\mleft(x_1, \ldots, x_n\mright)\right).\]

  Using the fact that $\mathrm{ZF} = \mathrm{IZF} + \mathrm{EM}$ as an intuitionistic theory, we can say that the double negation of every axiom of $\mathrm{ZF}$ holds in $T$. Thus, $T$ must also prove the double negation of every theorem of $\mathrm{ZF}$; more specifically, $T \vdash \neg \neg \neg \varphi$. Since intuitionistic logic admits triple-negation elimination, it follows that $T \vdash \neg \varphi$ and thus the inconsistency of $T$. Therefore, by contradiction, we must have $\mathrm{IZF} \nvdash \Ord \subseteq L$.
\end{proof}

Unfortunately, no such consistency results have been known, and we hope that further research will be able to close the problem regarding $\Ord \subseteq L$ in intuitionistic set theories in this direction.
