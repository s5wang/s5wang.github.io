...
Now, before proceeding to treat antichains, we observe that the classical way to defines this notion, i.e.
...
\begin{proof}
  ...
  Additionally, it is easy to check with the explcit constructions in \autoref{cor:perp-to-pow} and \autoref{prop:perp-to-ord-seq-union} that the values of $f_{x, \gamma}\mleft(y\mright)$ and $f_{y, \gamma}\mleft(y\mright)$ only depend on the values of $f_{x, \delta}\mleft(z\mright)$ and $f_{y, \delta}\mleft(z\mright)$ respectively for any $\delta \in \gamma$ and $z \in y$ such that $z \in \tc{x}_\delta$. We again use an induction on $y$, and notice that the inductive hypothesis implies that for such $\delta$ and $z$ we always have
